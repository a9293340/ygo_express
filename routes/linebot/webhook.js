...
const { reptileTargetUrl } = require('../../config/reptile/reptileTargetUrl.js');
...
const getJudRulesLink = async text => {
  ...
  if (lens === 1) return `https://www.db.yugioh-card.com${$('.link_value').attr('value')}`;
...
async function handleEvent(event) {
  ...
  if (event.source.type === 'user') {
    if (!userWhiteId.find(el => el === event.source.userId)) return Promise.resolve(null);
  }
  ...
  if (event.type === 'join' && event.source.type === 'group') {
    ...
    return client.replyMessage(event.replyToken, [{ type: 'text', text: '請於10秒鐘輸入通行令' }]);
  }
  ...
  if (search || price) {
    ...
    if (Number.isInteger(parseInt(tarText)) && search) {
      ...
      jud = 'n';
      // 中文
    } else if ((isChinese(tarText) || tarText.split(' ').length > 1) && search) {
    ...
    if (cardId.length) {
      ...
      if (search) {
        ...
        replyText += `屬性 : ${card.attribute} / 種族 : ${card.race ? card.race : '-'} / 等級 : ${
          card.star ? card.star : '-'
        }\n`;
        ...
        const priceText = prices.map(
          el => `版本 : ${el.rarity} / 均價 : ${el.price_avg} / 最低價 : ${el.price_lowest}\n`,
        );
        replyText += `卡片名稱 : ${card.name}\n`;
    ...
    } else replyText = '無此字段相關的卡片，可能翻譯問題(Ex: 如光道/光之領主)，請嘗試其他譯名';
  } else if (rules) {
    const tarText = event.message.text.substr(3);
    // console.log(tarText);
