...
async function handleEvent(event) {
	...
	if (search || price) {
		...
		if (/^[0-9]+$/.test(tarText) && search) {
			...
			(isChinese(tarText) || tarText.split(" ").length > 1) &&
			search
		) {
		...
		// 卡片名稱
		else if (containsJapanese(tarText)) {
			data = await MongooseCRUD("R", "jurisprudence", {
				name_jp_h: tarText,
			});
			if (!data.length)
		...
		// 卡號
			...
			console.log(tarText);
			const card = await MongooseCRUD("R", "cards", { id: tarText });
			console.log(card);
			if (card.length)
				data = await MongooseCRUD("R", "jurisprudence", {
		...
		if (data.length) {
			replyText += `卡片密碼 : ${data[0].number}\n`;
			replyText += `卡片名稱 : ${data[0].name_jp_h}\n`;
			replyText += `效果 : ${data[0].effect_jp}\n`;
			replyText += `QA連結 : ${data[0].jud_link} \n`;
		} else replyText += "找不到相應的QA資料";
