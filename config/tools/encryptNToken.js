  ...
  try {
    ...
    // 非合格路徑則回傳無權限
      ...
      if (typeof tokenReq !== 'string' || typeof token !== 'string') next(10003);
        ...
        // console.log("pass");
        const jud = req.path.indexOf('/deck/') !== -1 ? 'frontend_token' : 'backend_token';
        MongooseCRUD('R', jud, { token }).then(async (arr, err) => {
          ...
          if (err || arr.length > 1) next(err || 10004);
            ...
            if (!req.error_code) {
              await MongooseCRUD('Uo', jud, { tokenReq, token }, { date: new Date() });
              next();
...
const makeToken = async (type, removeToken, account, name = '') => {
  const token = v4();
...
const encryptRes = tar => CryptoJS.AES.encrypt(JSON.stringify(tar), 'C8763').toString();
